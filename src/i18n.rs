...
const PT_BR: &[(&str, &str)] = &[
    ...
    ("Empty goes back to the automatic name.", "Vazio volta ao nome automático."),
    ...
    ("Opens a Claude Code session with the output already in context, in a window of its own.",
     "Abre uma sessão do Claude Code com a saída já no contexto, em uma janela própria."),
    ...
    ("Could not start claude: {}. The output is in {}",
     "Não foi possível iniciar o claude: {}. A saída está em {}"),
    ...
    ("Keeps the commands typed at an IRIS prompt in history.txt, so Up reaches back past the session that is open now. Off keeps recall working within the session and writes nothing to disk. Either way, Up and Down are the recall, and lines sent by a macro or an IRIS helper are never offered back.",
     "Guarda os comandos digitados no prompt do IRIS em history.txt, de modo que a seta acima alcança sessões anteriores à atual. Desligado mantém a recuperação apenas dentro da sessão e não grava nada em disco. De todo modo, as setas acima e abaixo são a recuperação, e linhas enviadas por macro ou por um utilitário IRIS nunca são oferecidas de volta."),
    ...
    // Settings: window.
    ("Use the system title bar", "Usar a barra de título do sistema"),
    ("Off by default: the app draws its own, which frees the row the system bar would take.",
     "Desligado por padrão: o aplicativo desenha a sua própria, liberando a linha que a barra do sistema ocuparia."),
    ...
    ("Takes effect the next time the app starts.", "Passa a valer na próxima inicialização do aplicativo."),
    ...
    ("A UNC share, mapped drive, or local copy. Leave empty for none.                      Your personal macros are edited in the Macros panel.",
     "Um compartilhamento UNC, unidade mapeada ou cópia local. Deixe vazio para nenhum.                      Suas macros pessoais são editadas no painel Macros."),
    ("Found.", "Encontrado."),
    ...
    ("Log in automatically", "Efetuar login automaticamente"),
    ("Autologon needs a username.", "O login automático precisa de um usuário."),
    ("Default", "Padrão"),
    ("Forget", "Esquecer"),
    ("Save", "Salvar"),
    ("Cancel", "Cancelar"),
    ...
    // Macros panel and its editor.
    ("Filter", "Filtrar"),
    ("No macros defined.", "Nenhuma macro definida."),
    ("Add them below, or configure the organization file in Settings.",
     "Adicione-as abaixo, ou configure o arquivo da organização em Configurações."),
    ("New in group", "Nova no grupo"),
    ("Shortcut", "Atalho"),
    ...
    ("For a body that carries a password. Keeps it out of the macro panel; IRIS still echoes what it is sent.",
     "Para um corpo que carrega uma senha. Mantém-na fora do painel de macros; o IRIS ainda ecoa o que recebe."),
    ("Edit macro", "Editar macro"),
    ("Edit", "Editar"),
    ("Delete", "Excluir"),
    ("Removes it from your personal macro file.", "Remove-a do seu arquivo pessoal de macros."),
    ...
    ("UTF-8 double-encoded via CP850 (repair)", "UTF-8 codificado em dobro via CP850 (correção)"),
    ("Class", "Classe"),

    // Export.
    ("Export output", "Exportar saída"),
    ("Screen", "Tela"),
    ("Everything", "Tudo"),
    ...
    ("Reloaded {} macros.", "{} macros recarregadas."),
